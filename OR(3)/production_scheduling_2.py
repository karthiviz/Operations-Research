...
x = [[makespan.addVar(ub=1.0, vtype=GRB.CONTINUOUS, name="x"+str(j+1)+str(i+1)) for i in jobs] for j in machines]
...
#constraints
makespan.addConstrs((quicksum(x[i][j] for i in machines) == 1 for j in jobs), "Job-Machine unique assignment")
...
makespan.addConstrs(((quicksum(x[i][j] * processing_times[j+1] for j in jobs) <= w) for i in machines), "Available Uptimes")
makespan.update()
